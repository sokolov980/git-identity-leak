...
def build_identity_graph(signals):
    ...
    for s in signals:
        ...
        if username and stype not in ("REPO_SUMMARY","INACTIVITY_SCORE","CONTRIBUTIONS_YEAR","CONTRIBUTION_TOTAL","CONTRIBUTION_TIME_PATTERN","CONTRIBUTION_HOURLY_PATTERN","FOLLOWER_USERNAME","FOLLOWING_USERNAME","MUTUAL_CONNECTION"):
    ...
    if username:
        for s in signals:
            if s["signal_type"] in ("FOLLOWER_USERNAME","FOLLOWING_USERNAME","MUTUAL_CONNECTION"):
                G.add_edge(f"USERNAME:{username}", f"{s['signal_type']}:{s['value']}", relation="SOCIAL_LINK")
    ...
    if total_signal:
        ...
        for y in contrib_years:
            G.add_edge("CONTRIBUTION_TOTAL", f"CONTRIBUTIONS_YEAR:{y['meta']['year']}", relation="YEARLY")
    ...
    if daily_signal:
        for d in daily_signal["value"]:
            ...
            G.add_node(node_id,count=d["count"],signal_type="CONTRIBUTIONS_YEARLY_DATES")
            if username:
                G.add_edge(f"USERNAME:{username}", node_id, relation="DAILY_CONTRIBUTION")
